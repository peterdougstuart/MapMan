...
class ObjCClass (object):
	...
	def get_names(cls, prefix=None):
		...
		for i in xrange(num_classes):
			class_names.append(class_getName(buffer[i]))
		filtered_list = class_names if prefix is None else filter(lambda x: x.startswith(prefix), class_names)
...
class ObjCInstance (object):
	...
	def __new__(cls, ptr):
		...
		if isinstance(ptr, (ui.View, ui.Image, ui.ButtonItem, ui.Path, ui.Touch)):
			ptr = ptr._objc_ptr
		...
		if not method:
			...
			for possible_sel_name in possible_selector_names:
				...
				if method:
					self.sel_name = possible_sel_name
					break
		if not method and ((method_name.startswith('set') and method_name.endswith(':')) or method_name.find(':') == -1):
...
def ns(py_obj):
	...
	if isinstance(py_obj, (ui.View, ui.Image, ui.ButtonItem, ui.Path, ui.Touch)):
		return ObjCInstance(py_obj)
