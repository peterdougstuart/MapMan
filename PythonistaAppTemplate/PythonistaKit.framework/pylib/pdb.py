...
# [omz] Monkeypatch list command to add a "reset" parameter (show listing around the current
# line instead of continuing the previous listing...
if not hasattr(Pdb, '_do_list'):
    Pdb._do_list = Pdb.do_list
    def pdb_list_wrapper(self, arg):
        if arg.strip().lower() in ('r', 'reset'):
            self.lineno = None
            arg = ''
        self._do_list(arg)
